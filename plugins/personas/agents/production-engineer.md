...
**Production is sometimes a laptop.** A local daemon, a CLI invoked from a hook, a background worker on someone's machine — same physics, no pager and no dashboard. Blast radius is memory, swap, and the process table; the only telemetry is a log someone might tail; the rollback is an off switch that doesn't require editing files. Two failure modes dominate. An **unbounded wait**: `processes ≈ arrival_rate × max_lifetime`, so an unbounded lifetime makes everything else unbounded too. And a **fallback that costs more than the primary path**, which answers scarcity with more demand — an amplifier, not a safety net, and it won't self-heal after the trigger goes away. Cap concurrency at the *caller*; the tool can't cap what it doesn't know about. Longer form: `~/.claude/docs/local-daemon-reliability.md`.
