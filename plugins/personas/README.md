...
- **Craft & analysis personas** — deep expertise or a specific analytical lens you summon to do or examine work (`analyst`, `librarian`, `negotiator`, `rubyist`, `rustacean`).
...
- **`negotiator`** — a coach and sparring partner for real negotiations: business contracts, hiring and comp (either side), and getting your ask onto another team's roadmap. *Getting to Yes* (interests, BATNA, criteria) sets what to aim at; Voss's tactical empathy (labels, calibrated questions) sets how to say it — trimmed for relationships you'll live with afterwards. Preps, role-plays the other side, decodes a live offer, and debriefs. Won't script a lie. Keeps a local case file per negotiation under `~/.claude/docs/negotiations/` (only with your OK) and picks it back up when you return. Trigger on intent ("prep me for this," "how do I answer this offer," "role-play the other side").
...
- **`rustacean`** — a senior Rust expert with taste. Designs ownership instead of appeasing the borrow checker, makes invalid states unrepresentable, and keeps the public surface small — the error strategy (a concrete enum once a crate has outside consumers, `anyhow` with context until then), what's `pub`, where the module boundary falls, whether a `.clone()` is a symptom. Deliberately skips what `clippy -D warnings` already catches and spends its judgment where the compiler has no opinion. Reviews Rust *or* writes it. Trigger on intent ("write this in Rust," "review this Rust," "why am I cloning here").
...
- **`performance-engineer`** — "where does the time actually go?" Finds the cost rather than guessing it: budget first, marginal cost over totals, structural before algorithmic — and distrusts its own instinct, because the expensive thing is usually allocation, contention or redundant setup rather than the algorithm everyone is staring at. Prices the clever fix before building it, and records the rejections. (Knuth on premature optimization, Gregg's USE method; see [`references/where-the-time-goes.md`](./references/where-the-time-goes.md).)
